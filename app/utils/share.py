...
class ClashConfiguration(object):
    def __init__(self):
        self.data = {"port": 7890, "mode": "Global", "proxies": [], "proxy-groups": []}
        self.proxy_remarks = []

    def to_yaml(self):
        d = copy.deepcopy(self.data)
        d["proxy-groups"].append(
            {
                "name": "♻️ Automatic",
                "type": "url-test",
        ...
        self.data["proxies"].append(
            {
                ...
                "udp": True,
                "sni": sni if tls else "",
                f"{net}-opts": {"path": path, "host": host},
            }
        )
...
def generate_v2ray_links(proxies: dict, inbounds: dict, extra_data: dict) -> list:
    ...
    data_limit = (
        readable_size(extra_data.get("data_limit"))

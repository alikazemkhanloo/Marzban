...
class BotKeyboard:
    ...
    @staticmethod
    def user_menu(
        user_info, with_back: bool = True, page: int = 1, view_user: bool = False
    ):
        keyboard = types.InlineKeyboardMarkup()
        if view_user:
            keyboard.add(
                types.InlineKeyboardButton(
                    text="View User", callback_data=f"user:{user_info['username']}:1"
                )
            )
        keyboard.add(
            types.InlineKeyboardButton(
            ...
            types.InlineKeyboardButton(
                text="Delete User", callback_data=f"delete:{user_info['username']}"
            ),
        )
        if with_back:
            keyboard.add(
                types.InlineKeyboardButton(text="Back", callback_data=f"users:{page}")
            )
        return keyboard
    ...
    def select_protocols(selected: list):
        ...
        for protocol in protocols:
            ...
            keyboard.row(*row)
        keyboard.add(
            types.InlineKeyboardButton(text="Done", callback_data="confirm:add_user")
        )

...
def report(message: str, parse_mode="html", keyboard=None):
    if bot and TELEGRAM_ADMIN_ID:
        try:
            bot.send_message(
                TELEGRAM_ADMIN_ID, message, parse_mode=parse_mode, reply_markup=keyboard
            )
...
def report_new_user(
    user_id: int, username: str, by: str, expire_date: int, usage: str, proxies: list
):
    text = """
...
def report_user_modification(
    username: str, expire_date: int, usage: str, proxies: list, by: str
):
    text = """
...
└ Status: <b>{status}</b>
    """.format(
        username=escape_html(username), status=status.capitalize()
    )
    return report(text)
